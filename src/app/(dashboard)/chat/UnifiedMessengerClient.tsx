...
import { saveDirectMessage, saveModChatMessage, markDirectMessagesAsReadAction } from "@/app/actions/messages";
...
export default function UnifiedMessengerClient({ currentUser, platformUsers, initialDMs, initialModMessages = [] }: UnifiedClientProps) {
  ...
  // 🚀 THE UNREAD FLUSH HANDLER:
  // Wipes unread alert counts instantly inside Neon PostgreSQL when clicking a chat card!
  const handleSelectContactChatLine = async (contact: Contact) => {
    ...
    try {
      ...
      if (result && result.success) {
        // Optimistically set messages as read in client state immediately for rapid UX
        setPrivateMessages((prev) => 
          prev.map((msg) => msg.senderId === contact.id ? { ...msg, isRead: true } : msg)
        );
        router.refresh();
      }
    } catch (err) {
      console.error("Failed to flush message unread layers:", err);
    }
  };
  ...
  const handleSendMessageSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedChannel === "PUBLIC_LOUNGE" || selectedChannel === "MOD_CHAT") {
      ...
      if (selectedChannel === "MOD_CHAT") {
        ...
        saveModChatMessage(cleanText, currentUser.id).catch((err) => console.error(err));
      } else {
    ...
    <div className="flex h-full w-full bg-white select-none relative overflow-hidden max-h-full">
      ...
      <div 
        className={`h-full flex flex-col bg-white border-r border-gray-100 transition-all duration-300 ease-in-out shrink-0 ${
          isChatSelected ? "hidden md:flex md:w-72 lg:w-80 px-4" : "w-full md:w-72 lg:w-80 px-4"
        }`}
      >
        <div className="p-4 border-b border-gray-100 shrink-0 text-left">
          <h2 className="font-black text-sm text-gray-900 uppercase tracking-wide">Conversations</h2>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-3 space-y-1.5">
          <button
            ...
            className={`w-full flex items-center space-x-3 p-3 rounded-2xl transition text-left border ${
              selectedChannel === "PUBLIC_LOUNGE" ? "bg-rose-500 text-white border-rose-600 shadow-sm" : "hover:bg-gray-50 text-gray-700 border-transparent"
            }`}
          >
          ...
          {hasStaffPrivileges && (
            <button
              ...
              className={`w-full flex items-center space-x-3 p-3 rounded-2xl transition text-left border ${
                selectedChannel === "MOD_CHAT" ? "bg-purple-600 text-white border-purple-700 shadow-sm" : "bg-purple-50/40 hover:bg-purple-50 text-purple-700 border-transparent"
              }`}
            >
              <span className="text-xl shrink-0">🛡️</span>
              <div className="min-w-0 flex-1 text-left">
                <span className="font-black text-xs block leading-tight">Staff Mod Chat</span>
                <span className={`text-[10px] block font-bold ${selectedChannel === "MOD_CHAT" ? "text-purple-100" : "text-purple-500"}`}>🔒 Restricted Channel</span>
              </div>
            </button>
          )}

          <div className="text-[10px] uppercase font-black tracking-wider text-gray-400 px-3 pt-4 pb-1 text-left">Online Users</div>
          ...
          {activePrivateLinesList.map((contact) => {
            ...
            // 🎯 THE LIVE COHESION CALCULATOR: Checks message arrays for unread states belonging to this explicit sender handle!
            const specificContactUnreadCount = privateMessages.filter(
              (m) => m.senderId === contact.id && m.recipientId === currentUser.id && m.isRead === false
            ).length;

            return (
              <div key={contact.id} className={`w-full flex items-center justify-between p-2 rounded-2xl transition border ${isSelected ? "bg-rose-50 border-rose-100" : "border-transparent hover:bg-gray-50/60"}`}>
                <button
                  onClick={() => handleSelectContactChatLine(contact)}
                  className="flex items-center space-x-3 text-left min-w-0 flex-1"
                >
                  <div className="relative shrink-0 p-[2px]">
                    <img 
                      src={contact.avatarUrl || "https://ufs.sh"}  
                      alt="" 
                      className={`w-9 h-9 rounded-full object-cover shadow-sm border-2 ${
                        resolvedLiveStatus === "ONLINE" ? "border-green-500" :
                        resolvedLiveStatus === "AWAY" ? "border-yellow-500" :
                        resolvedLiveStatus === "BUSY" ? "border-red-500" : "border-gray-200"
                      }`} 
                    />
                  ...
                  <div className="min-w-0 flex-1 text-left">
                    ...
                    <span className="text-[10px] text-gray-400 font-semibold block truncate leading-tight">@{contact.username}</span>
                    ...
                    {(contact.genderIdentity || contact.location) && (
                      <div className="flex items-center flex-wrap gap-1 mt-1 max-w-full truncate select-none">
                        {contact.genderIdentity && <span className="bg-rose-50 text-rose-500 font-bold px-1.5 py-0.5 rounded text-[8px] tracking-wide uppercase shrink-0">✨ {contact.genderIdentity}</span>}
                        {contact.location && <span className="bg-gray-100 text-gray-500 font-medium px-1.5 py-0.5 rounded text-[8px] tracking-wide truncate max-w-[100px]">📍 {contact.location}</span>}
                      </div>
                    )}
                  </div>

                  {/* 🚀 SIDEBAR TICK BADGE: Draws numerical ring indicators live! */}
                  {specificContactUnreadCount > 0 && !isSelected && (
                    <span className="bg-rose-500 text-white font-black text-[9px] px-2 py-0.5 rounded-full shadow-xs shrink-0 ml-2 animate-scale-up">
                      {specificContactUnreadCount}
                    </span>
                  )}
                </button>
                ...
                <div className="relative" ref={activeMenuId === contact.id ? optionMenuRef : null}>
                  ...
                  {activeMenuId === contact.id && (
                    <div className="absolute right-0 top-full mt-1 w-44 bg-white border border-gray-200 rounded-xl shadow-xl p-1 z-50 animate-scale-up divide-y divide-gray-50 text-xs">
                      <Link href={`/${contact.username}`} className="w-full block px-3 py-2 text-left font-black text-gray-700 hover:bg-rose-50 hover:text-rose-500 rounded-lg transition">👤 View Profile</Link>
                      <button type="button" onClick={async () => { setActiveMenuId(null); const res = await ignoreUserAction(contact.id); if (res.success) alert(`Muted @${contact.username}.`); }} className="w-full block px-3 py-2 text-left font-semibold text-amber-600 hover:bg-amber-50 rounded-lg transition">⏳ Ignore for 10 mins</button>
                      <button type="button" onClick={async () => { setActiveMenuId(null); const res = await blockUserAction(contact.id); if (res.success) alert(`Blocked @${contact.username}.`); }} className="w-full block px-3 py-2 text-left font-semibold text-red-600 hover:bg-red-50 rounded-lg transition">🚫 Block User Account</button>
                    </div>
                  )}
                </div>
              </div>
      ...
      <div className={`flex flex-col bg-gray-50/50 h-full w-full min-h-0 flex-1 relative ${!isChatSelected ? "hidden md:flex" : "flex"}`}>
        <div className="p-4 py-2 bg-white border-b border-gray-200 flex items-center justify-between shrink-0 text-left w-full h-14 min-h-14 overflow-hidden">
          <div className="flex items-center space-x-3 min-w-0 flex-1">
            {isChatSelected && (
              <button type="button" onClick={() => setMobileViewState("ROOMS")} className="block md:hidden font-black uppercase text-[10px] tracking-wider text-rose-500 bg-rose-50 hover:bg-rose-100 border border-rose-100/60 px-3 py-1.5 rounded-xl shrink-0 transition">◀ Rooms</button>
            )}
            ...
            {selectedChannel === "PUBLIC_LOUNGE" ? (
              <div className="flex items-center justify-between w-full min-w-0 pr-1">
                ...
                <span className="bg-green-50 border border-green-100 text-green-600 text-[10px] font-black px-2 py-0.5 rounded-md shrink-0 whitespace-nowrap ml-2">● {activePresence.filter(u => u.currentRoom === "PUBLIC_LOUNGE").length} Dolls Here</span>
              </div>
              ...
              <div className="flex items-center justify-between w-full min-w-0 pr-1">
                ...
                <span className="bg-purple-50 border border-purple-100 text-purple-600 text-[10px] font-black px-2 py-0.5 rounded-md shrink-0 whitespace-nowrap ml-2">🔒 Secure</span>
              </div>
            ) : activeContact ? (
              <div className="flex items-center justify-between w-full min-w-0 pr-1">
                <div className="flex items-center space-x-2 min-w-0">
                  <img src={activeContact.avatarUrl || "https://ufs.sh"} className="w-7 h-7 rounded-full object-cover border border-gray-100 shadow-sm shrink-0" />
                  <span className="font-black text-xs text-gray-900 block truncate leading-none">{activeContact.displayName}</span>
                </div>
                <span className="bg-rose-50 border border-rose-100 text-rose-500 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md shrink-0 whitespace-nowrap ml-2">🔒 Private Message</span>
              </div>
            ) : null}
          </div>
        </div>
            ...
            {selectedChannel === "PUBLIC_LOUNGE" || selectedChannel === "MOD_CHAT" ? (
                ...
                (selectedChannel === "MOD_CHAT" ? modMessages : publicMessages).map((msg) => {
                  ...
                  return (
                    <div key={msg.id} className={`flex items-end gap-2 max-w-[85%] ${isMe ? "ml-auto flex-row-reverse" : "mr-auto"} animate-fade-in`}>
                      <img src={msg.user?.avatarUrl || "https://ufs.sh"} alt="" className="w-7 h-7 rounded-full object-cover shrink-0 border border-gray-100 dark:border-gray-800 shadow-sm bg-white dark:bg-gray-900" />
                      <div className="space-y-0.5 text-left max-w-full">
                        ...
                        <div className={`p-3 rounded-2xl shadow-sm text-xs font-medium whitespace-pre-wrap leading-relaxed ${isMe ? "bg-rose-500 text-white rounded-br-none" : "bg-white text-gray-800 border border-gray-100 rounded-bl-none"}`}>
              ...
              activeChatFeedDMs.length === 0 ? (
                <div className="my-auto flex flex-col items-center justify-center text-gray-400">
                  ...
                  <p className="text-[11px] mt-0.5">Your conversations are fully sandboxed.</p>
                </div>
              ) : (
                  ...
                  return (
                    <div key={msg.id} className={`flex items-end gap-2 max-w-[80%] ${isMe ? "ml-auto flex-row-reverse" : "mr-auto"} animate-fade-in`}>
                      <div className="space-y-0.5 text-left max-w-full">
                        ...
                        <span className={`text-[9px] text-gray-400 font-bold block px-1.5 ${isMe ? "text-right" : "text-left"}`}>{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                    </div>
                  );
                })
        ...
        <form onSubmit={handleSendMessageSubmit} className="p-4 bg-white border-t border-gray-200 flex items-center gap-2 shrink-0 w-full">
          <input id="chat-message-input" type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder={selectedChannel === "PUBLIC_LOUNGE" ? "Say something..." : selectedChannel === "MOD_CHAT" ? "Say something..." : `Message @${activeContact?.username}...`} className="flex-1 border border-gray-200 rounded-xl p-3 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-rose-400 focus:bg-white text-xs font-medium text-gray-800 transition" />
          <button type="submit" className={`font-black px-5 py-3 rounded-xl transition shadow-sm text-xs tracking-wide text-white shrink-0 ${selectedChannel === "MOD_CHAT" ? "bg-purple-600 hover:bg-purple-700" : "bg-rose-500 hover:bg-rose-600"}`}>Send</button>
        </form>
      </div>
    </div>
  );
