  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      setIsSaving(false);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                <div key={idx} className={`p-3.5 rounded-2xl flex items-center justify-between border ${isDarkMode ? "bg-gray-950/20 border-gray-800" : "bg-gray-50/50 border-gray-100"}`}>
                  ...
                  <button type="button" onClick={() => notif.setter(!notif.state)} className={`w-10 h-5 flex items-center rounded-full p-0.5 transition-colors duration-300 shrink-0 ${notif.state ? "bg-rose-500 justify-end" : "bg-gray-300 justify-start"}`}><span className="bg-white w-3.5 h-3.5 rounded-full shadow-xs block" /></button>
                </div>
              ))}
            </div>
          </section>

          {/* SECTION 4: DYNAMIC BLOCK LIST LINK PANEL */}
          <section className="space-y-4 border-t border-gray-100 dark:border-gray-800 pt-6">
          ...
          {/* 🚀 MUTATION TRIGGER: Bound straight to handleSaveSettings! */}
          <div className="border-t border-gray-100 dark:border-gray-800 pt-6 flex justify-end">
            <button 
              ...
              className={`text-white font-black text-xs uppercase tracking-widest px-8 py-3.5 rounded-xl transition shadow-md ${
                isSaving ? "bg-gray-400 cursor-not-allowed opacity-50" : "bg-rose-500 hover:bg-rose-600 transform hover:-translate-y-[1px] active:translate-y-0"
              }`}
            >
